...
public class ChatListener implements Listener {
	...
	public void playerSpeaks(AsyncPlayerChatEvent event) {
		...
		if (plugin.getConfigManager().chatFeaturesEnabled == false) {
			...
			if (plugin.getServer().getPluginManager().isPluginEnabled("Factions") == true) {
			...
			if (plugin.getConfigManager().isWorldEnabled(senderWorld.getName()) == true) {
				...
				for (Player recipient : recipients) {
					...
					//  (local chat)
					if (recipient.getLocation().distance(sender.getLocation()) <= plugin.getConfigManager().clearChatRange) {
					...
					} else if (recipient.getLocation().distance(sender.getLocation()) <= plugin.getConfigManager().garbledChatRange) { 
				...
				for (Player recipient : recipients) {
					...
					if (plugin.getConfigManager().isWorldEnabled(recipient.getWorld().getName()) == true) {
