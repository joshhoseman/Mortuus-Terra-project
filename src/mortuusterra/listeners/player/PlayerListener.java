...
public class PlayerListener implements Listener {
	...
	public void playerIsAttackedByPlayer(EntityDamageByEntityEvent event) {
		...
		if (event.getDamager() instanceof Player && event.getEntity() instanceof Player) {
			...
			// plugin.getLogger().info("Victim Join Date: " + victimObject.joinDate + " | Server Time: " + victim.getWorld().getFullTime());
	...
	public void playerLogsIn(PlayerJoinEvent event) {
		...
		if (plugin.getPlayerManager().getPlayerObjectByPlayerName(event.getPlayer().getName()) == null) {
			...
			plugin.getPlayerManager().addPlayer(event.getPlayer().getName(), event.getPlayer().getWorld().getFullTime(), 20, 20, 20);
	...
	public void playerMarksProtection(PlayerInteractEvent event) {
		...
		if (event.getPlayer().isOp() == false && plugin.getPermissionsManager().playerHasPermissions(event.getPlayer(), "mt.admin") == false && plugin.getPermissionsManager().playerHasPermissions(event.getPlayer(), "mt.create.*") == false && plugin.getPermissionsManager().playerHasPermissions(event.getPlayer(), "mt.create.protection") == false) {
	...
	public void playerUsesGeigerCounter(PlayerInteractEvent event) {
		...
		if ((event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) && event.getPlayer().getItemInHand().getTypeId() == plugin.getConfigManager().geigerCounterItemId) {
	...
	*/
	...
	public void playerSearchesForSupplyDropSignal(PlayerInteractEvent event) {
		...
		if (plugin.getConfigManager().supplyDropsEnabled == true && event.getPlayer().isSneaking() == false && (event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) && event.getPlayer().getItemInHand().getType().equals(Material.COMPASS)) {

			SupplyDrops closestSupplyDrop = plugin.getSupplyDropManager().getNearestSupplyDrop(event.getPlayer().getLocation());
