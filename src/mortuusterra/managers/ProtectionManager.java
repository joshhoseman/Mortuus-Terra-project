...
import mortuusterra.objects.ProtectionObject;
...
public class ProtectionManager {
	...
	public void saveProtectionsToDisk() {
		...
		try {
			...
			File protectionsFile = new File(plugin.getDataFolder() + File.separator + "protections.txt");
			protectionsFile.delete();
	
			protectionsFile = new File(plugin.getDataFolder() + File.separator + "protections.txt");
		...
		plugin.getLogger().info("Saving Protections To Disk");

		try {
			
			PrintWriter protectionsFile = new PrintWriter(plugin.getDataFolder() + File.separator + "protections.txt");
	...
	public void loadProtectionsFromDisk() {
		...
		plugin.getLogger().info("Loading Protections From Disk");
		...
		try {
			...
			while (inputFile.hasNextLine()) {
				...
				Location protectionMarkOne = new Location(plugin.getServer().getWorld(protectionArray[1]), Double.valueOf(protectionArray[2]), Double.valueOf(protectionArray[3]), Double.valueOf(protectionArray[4]));
				Location protectionMarkTwo = new Location(plugin.getServer().getWorld(protectionArray[5]), Double.valueOf(protectionArray[6]), Double.valueOf(protectionArray[7]), Double.valueOf(protectionArray[8]));
	...
	public void ensureProtectionsFileExists() {
		...
		plugin.getLogger().info("Creating Protections File");
