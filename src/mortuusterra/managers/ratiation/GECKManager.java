...
private final Main plugin;
	...
	public GECKManager(Main plugin) {
	...
	public void saveGECKsToDisk() {
		...
		try {
			...
			File gecksFile = new File(plugin.getDataFolder() + File.separator + "gecks.txt");
			gecksFile.delete();
	
			gecksFile = new File(plugin.getDataFolder() + File.separator + "gecks.txt");
		...
		plugin.getLogger().info("Saving GECKs To Disk");

		try {
			
			PrintWriter gecksFile = new PrintWriter(plugin.getDataFolder() + File.separator + "gecks.txt");
	...
	public void loadGECKsFromDisk() {
		...
		plugin.getLogger().info("Loading GECKs From Disk");
		...
		try {
			...
			while (inputFile.hasNextLine()) {
				...
				Location geckLocation = new Location(plugin.getServer().getWorld(geckArray[0]), Double.valueOf(geckArray[1]), Double.valueOf(geckArray[2]), Double.valueOf(geckArray[3]));
	...
	public void ensureGECKsFileExists() {
		...
		plugin.getLogger().info("Creating GECKs File");
