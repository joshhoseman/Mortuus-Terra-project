...
public class FalloutShelterManager {
	...
	public void saveFalloutSheltersToDisk() {
		...
		try {
			...
			PrintWriter falloutSheltersFile = new PrintWriter(
					plugin.getDataFolder() + File.separator + "fallout-shelters.txt");
	...
	public void loadFalloutSheltersFromDisk() {
		...
		try {
			...
			while (inputFile.hasNextLine()) {
				...
				Location falloutShelterLocation = new Location(plugin.getServer().getWorld(falloutShelterArray[0]),
						Double.valueOf(falloutShelterArray[1]), Double.valueOf(falloutShelterArray[2]),
						Double.valueOf(falloutShelterArray[3]));
